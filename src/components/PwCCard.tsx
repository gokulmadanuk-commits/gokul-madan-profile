  ...
  return <Card className={`shadow-lg ${className || ''}`}>
      <CardContent className="p-6 h-full flex flex-col">
            ...
            {logos.length <= 3 ? <div className="flex items-center justify-around">
                ...
                <div className="flex items-center justify-around">
                  {logos.slice(2).map((logo, index) => <div key={index + 2} className="flex items-center justify-center h-16 w-24">
                      <img src={logo.alt === "ATL Partners" ? "/lovable-uploads/4e34a33c-d575-4795-902a-a787b37e8eca.png" : logo.src} alt={logo.alt} className="max-h-full max-w-full object-contain" />
                    </div>)}
                </div>
        ...
        {tags && tags.length > 0 && <div className="flex gap-2 flex-wrap mt-auto">
            {tags.map((tag, index) => <span key={index} className="inline-block bg-[#393F4A]/50 text-white px-2 py-1 rounded-full text-xs font-medium">
                {tag}
              </span>)}
          </div>}
      </CardContent>
